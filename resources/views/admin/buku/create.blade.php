                    ...
                    <div class="card-body">
                        <form action="{{ route('buku.store') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select id="form-select-sm"
                                            class="form-select @error('author_id') is-invalid @enderror" name="author_id">
                                            {{-- <option value="">-- Pilih Penulis --</option> --}}
                                            @foreach ($penulis as $item)
                                                <option value="{{ $item->id }}">{{ $item->name ?? old('author_id') }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('author_id')
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="keterangan" cols="30" rows="2"
                                            ...
                                            placeholder="Tuliskan deskripsi buku">{{ old('keterangan') }}</textarea>
                                        @error('keterangan')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control @error('jumlah') is-invalid @enderror"
                                            name="jumlah" placeholder="10" value="{{ old('jumlah') }}">
                                        @error('jumlah')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="form-control-label">{{ __('Category Buku') }}</label>
                                        <select id="form-select-sm"
                                            class="form-select @error('category_id') is-invalid @enderror"
                                            name="category_id">
                                            <option value="">-- Pilih Catergory Buku --</option>
                                            @foreach ($category as $item)
                                                <option value="{{ $item->id }}">
                                                    {{ $item->name ?? old('category_id') }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('category_id')
